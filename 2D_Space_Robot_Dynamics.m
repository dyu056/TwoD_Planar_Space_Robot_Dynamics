...
generalized_force = [0; 0; 0; 0; 0.1]; % Constant torques for each joint, for example
...
[t, state] = ode45(@(t, state) Planar_Space_Robot_Dynamics(t, state, generalized_force, robot_dynamics_constants), tspan, initial_state);
...
%% Run the simulation
simOut = sim('SpaceRobotics', 'StopTime', '10');
...
% Plot Theta0
subplot(2, 3, 1);
plot(theta0.Time, (theta0.Data - theta0.Data(1)) .* theta0_direction * 180 / pi);
hold on
...
% Plot Theta2
subplot(2, 3, 3);
plot(theta2.Time, (theta2.Data - theta2.Data(1)) .* theta2_direction * 180 / pi);
hold on
